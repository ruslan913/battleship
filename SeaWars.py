...
class AutoShips:
    def __init__(self, offset):
        self.offset = offset 
        self.available_blocks = {(x, y) for x in range(1 + self.offset, 11 + self.offset)
                                 for y in range(1, 11)}
        self.ships_set = set()
        ...
        for _ in range(number_of_blocks):
            ...
            if not self.orientation:
                self.direction, x = self.__get_new_block_for_ship(
                    x, self.orientation, self.direction, ship_coordinates)
            else:
    ...
    def __get_new_block_for_ship(self, coor, direction, orientation, ship_coordinates):
        ...
        if (coor <= 1 - self.offset * (self.orientation - 1) and self.direction == -1) or (coor >= 10 - self.offset * (self.orientation - 1) and self.direction == 1):
            self.direction *= -1
...
def show_messeng_at_rect_center(message, rect, which_font = font, color = RED):
...
def ship_is_valid(ship_set, blocks_for_manial_drawing):
    return ship_set.isdisjoint(blocks_for_manial_drawing)
...
auto_buton = Button(auto_button_place, "АВТО", how_to_create_ships_message)
...
def main():
    ...
    rect_for_messages_and_button = (0, upper_margin + 11 * block_size, size[0], 5 * block_size)
    message_rect_for_drawing_ships = (undo_button.rect_for_draw[0] + undo_button.rect_for_draw[2], upper_margin + 11 * block_size, size[0] - (undo_button.rect_for_draw[0] + undo_button.rect_for_draw[2]), 4 * block_size) 
    ...
    human_ships_to_draw = []
    human_ships_set = set()
    used_block_for_manual_drawing = set()
    ...
    while ships_creation_not_decided:
        auto_buton.draw_button()
        manual_button.draw_button()
        auto_buton.change_color_on_hover()
        manual_button.change_color_on_hover()
        auto_buton.print_message_for_button()
        ...
        for event in pygame.event.get():
            ...
            elif event.type == pygame.MOUSEBUTTONDOWN and auto_buton.rect.collidepoint(mouse):
                print("Clickrd AUTO!", event.pos)
                human = AutoShips(15)
        ...
        screen.fill(WHITE, rect_for_messages_and_button)

    while ships_not_created:
        screen.fill(WHITE, rect_for_grids)
        computer_grid = Grid("COMPUTER", 0)
        human_grid = Grid("HUMMAN", 15)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if human_ships_to_draw:
                    ...
                    update_used_blocks(deleted_ship, used_block_for_manual_drawing.discard)
                ...
                if  15 < start_block[0] < 26 and 0 < start_block[1] < 11 and 15 < end_block[0] < 26 and 0 < end_block[1] < 11:
                    ...
                    if start_block[0] == end_block[0] and (end_block[1] - start_block[1]) < 4:
                        ...
                        show_messeng_at_rect_center("Корабль слишком большой!", message_rect_for_drawing_ships)
                else:
                    show_messeng_at_rect_center("Корабль за пределами сетки", message_rect_for_drawing_ships)
                if temp_ship:
                    temp_shit_set = set(temp_ship)
                    if ship_is_valid(temp_shit_set, used_block_for_manual_drawing):
                        if check_ships_numbers(temp_ship, num_ship_list):
                            num_ship_list[len(temp_ship) - 1] += 1
                            human_ships_to_draw.append(temp_ship)
                            human_ships_set |= temp_shit_set
                            update_used_blocks(temp_ship, used_block_for_manual_drawing.add)
                        else:
                            show_messeng_at_rect_center(f"Уже достаточно {len(temp_ship)} - палубных кораблей", message_rect_for_drawing_ships)
                    else:
                        show_messeng_at_rect_center("Корабли прикасаются", message_rect_for_drawing_ships)
    ...
    while not game_over:
        ...
        if not (dotted_set | hit_blocks):
            show_messeng_at_rect_center("Игра началась! Ваш ход", message_rect_computer)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                game_over = True
                ...
                show_messeng_at_rect_center(f"Ваш последний ход: {LETTERS[fired_block[0] - 1] + str(fired_block[1])}", message_rect_computer, color = BLACK)
        else:
            show_messeng_at_rect_center("Выстрел за пределы сетки", message_rect_computer)
        ...
        if computer_turn:
            ...
            show_messeng_at_rect_center(f"Последний ход компьютера: {LETTERS[fired_block[0] - 16] + str(fired_block[1])}", message_rect_human, color = BLACK)
        
        if not computer.ships_set:
            show_messeng_at_rect_center("Вы выйграли!", (0, 0, size[0], size[1], game_over_font))
        if not human_ships_set:
            show_messeng_at_rect_center("Вы проиграли!", (0, 0,size[0], size[1], game_over_font))
        pygame.display.update()
